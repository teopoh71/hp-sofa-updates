...
const zolano2628Modules = [
  {
    ...
    label: "2EL",
    meta: "1520mm",
    photo: "assets/generated/zolano/parts/MONTIERIZL2628-1ELT-top.png"
  },
  {
...
function renderVersionBadge() {
  ...
  const installedPatchName = localStorage.getItem(patchNameStorageKey) || currentAppVersion.patchVersionName || "";
...
function syncBuilderControlsVisibility() {
  ...
  if (zolanoModulePicker) zolanoModulePicker.hidden = isGridOnlyCatalog || !isZolanoModulePickerActive();
...
async function maybeClearAppCache() {
  ...
  const clearKey = "hp-sofa-cache-cleared-v250";
...
function getComboElectricSeatCount(combo) {
  ...
  return String(combo?.description || "")
    .split(/[+＋,，、]/)
    .filter((part) => /电动/.test(part))
    .length;
}

function isElectricSeatItem(item) {
  return /电动|\bRL\b/i.test(`${item?.configuration || ""} ${item?.description || ""} ${item?.model || ""}`);
}
...
function getActiveZolanoModules() {
...
function isZolanoModulePickerActive() {
  return Boolean(getActiveZolanoModuleSeries());
}

function syncZolanoModulePicker() {
  if (!zolanoModulePicker) return;
  const modules = getActiveZolanoModules();
  const activeSeries = getActiveZolanoModuleSeries();
...
function addZolano2897Module(moduleId) {
  if (!isZolanoModulePickerActive()) return;
...
function formatPieceTitle(item) {
  ...
  return `${isZolanoAddOnItem(item) ? "配件/加购 - " : ""}${title || item?.model || item?.series || ""}`;
}
...
function formatPieceDimensionLine(item) {
  ...
  return dimensions && dimensions !== "配件" ? dimensions : "";
}
