...
function ensureCatalogSwitchButtons() {
  ...
  const switchPanel = document.querySelector(".catalog-switch");
  const oldBackupButton = switchPanel?.querySelector('[data-brand-switch="backup"]');
  if (oldBackupButton) {
    oldBackupButton.dataset.brandSwitch = "coffeeTable";
    oldBackupButton.textContent = "\u8336\u51e0";
  }
  const coffeeTableButton = switchPanel?.querySelector('[data-brand-switch="coffeeTable"]');
  switchPanel?.querySelectorAll("button").forEach((button) => {
    if ((button.textContent || "").trim() === "\u5907\u7528") {
      button.dataset.brandSwitch = "coffeeTable";
      button.textContent = "\u8336\u51e0";
    }
  });
  if (switchPanel && coffeeTableButton) switchPanel.append(coffeeTableButton);
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    ...
    navigator.serviceWorker.register("sw.js?v=v1026-sw-cache-patch-fix")
...
async function maybeClearAppCache() {
  ...
  const clearKey = "hp-sofa-cache-cleared-v1026-sw-cache-patch-fix";
