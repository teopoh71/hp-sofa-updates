...
const zolanoThreeDigitCatalog = [
  ...
  },
  {
    ...
    dimensions: "",
    price: 16301,
    priceOptions: [16301, 20384, 23164],
    materials: ["M/F", "F/SA", "NuBuck"],
    priceFactor: 1,
    priceIsFinal: true,
...
function populatePieceSelect(select, items, selectFirst = false) {
  ...
  if (selectFirst && items.length) {
    const preferredItem = items.find((item) => Number(item?.price || 0) > 0
      || (Array.isArray(item?.priceOptions) && item.priceOptions.some((value) => Number(value) > 0)))
      || items[0];
...
function fillDefaultSingleCatalogSlot() {
  if (activeCatalogKey !== "chair") return false;
  const item = getSeriesItems().find((sofa) => Number(sofa?.price || 0) > 0
    || (Array.isArray(sofa?.priceOptions) && sofa.priceOptions.some((value) => Number(value) > 0)))
    || getSeriesItems()[0];
