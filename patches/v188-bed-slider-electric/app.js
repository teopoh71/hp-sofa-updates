...
function ensureDynamicQuickJumpGroup(catalogKey) {
  ...
  if (existingGroup) {
    if ((catalogKey === "nikator" || catalogKey === "bed") && !existingGroup.querySelector(".quick-jump-pages")) {
...
function appendQuickJumpButtons(group, catalogKey, items) {
  if (catalogKey === "nikator" || catalogKey === "bed") {
...
function getCatalogQuickJumpItems(catalogKey, catalog) {
  ...
  const items = catalog
    ...
    .slice(0, limit);
  if (catalogKey !== "nikator") return items;
  ...
  return [...pinned, ...items.filter((item) => !pinnedSet.has(item.series))];
}
