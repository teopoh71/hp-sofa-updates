...
function getCatalogCardPriceText(item) {
  ...
  return money.format(getRetailPrice(item.price || 0, 1, item.priceFactor || 1));
}
...
function fillDefaultSingleCatalogSlot() {
  if (!["chair", "diningChair"].includes(activeCatalogKey)) return false;
  const items = getSeriesItems();
  const item = items[0];
  const select = slotGrid.querySelector(".slot-select");
  if (!item || !select || items.length !== 1) return false;
