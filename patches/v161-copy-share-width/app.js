...
function populateRecommendationSelect() {
  ...
  combos.forEach((combo) => {
    ...
    option.textContent = formatComboOption(combo);
...
function formatComboButtonLabel(combo) {
  ...
  return [name, pieces ? `${pieces}\u4ef6` : "", sizeText].filter(Boolean).join(" ");
}
...
function getComboButtonDepthValue(combo) {
  ...
  const seriesDisplayDepths = {
    LE8807SF: 690
  };
  const seriesDisplayDepth = seriesDisplayDepths[String(combo?.series || "").toUpperCase()];
  if (seriesDisplayDepth) return seriesDisplayDepth;
...
function getSeriesRecommendations() {
  ...
  return displayCombos.filter((combo) => {
    ...
    if (activeCatalogKey === "zolano" && !isZolanoRecommendedCombo(combo)) return false;
    const key = getComboPartCodes(combo).join("+") || combo.id;
    if (seen.has(key)) return false;
    seen.add(key);
    if (widthFilter) {
      const width = getComboWidthValue(combo);
      if (!width || Math.abs(width - widthFilter.target) > widthFilter.tolerance) return false;
    }
...
function getZolanoGeneratedRecommendations(items) {
  ...
  const generated = [
    ...getZolano3776GeneratedRecommendations(items)
  ];
...
function isZolanoFullUnit(item) {
  ...
  return /\b\d+(?:\.\d+)?S\b/.test(config);
}

function isZolanoRecommendedCombo(combo) {
  if (!combo) return false;
  if (isGeneratedZolanoCombo(combo)) return true;
  const config = String(combo.configuration || "").toUpperCase();
  return config.includes("+") || /\b\d+(?:\.\d+)?S\b/.test(config);
}
...
function renderSetPreview() {
  ...
  const selected = [...slotGrid.querySelectorAll(".slot-select")]
    ...
    .filter(Boolean);
  const effectiveRecommendation = selectedRecommendation || findMatchingRecommendedCombo(selected);
...
function renderPhotoActions(item) {
  const photos = getItemGallery(item);
  const hasPhoto = photos.length || resolveItemPhoto(item);
  if (!hasPhoto) return "";
  return `
    <div class="photo-actions">
      <button class="photo-action-button" type="button" data-copy-model-photos>复制当前图片</button>
      <button class="photo-action-button" type="button" data-share-current-photo>发送当前图片</button>
    </div>
  `;
}
...
function bindPhotoActions(container) {
  const copyButton = container.querySelector("[data-copy-model-photos]");
  const shareButton = container.querySelector("[data-share-current-photo]");
  if (copyButton) {
    copyButton.addEventListener("click", async () => {
      const currentPhoto = getCurrentPhotoUrl(container);
      if (!currentPhoto) return;
      const copied = await copyCurrentPhotoToClipboard(currentPhoto, container.querySelector(".set-main-photo"));
      if (copied) {
        flashButtonText(copyButton, "已复制");
      } else {
        const shared = await sharePhoto(currentPhoto, container.querySelector(".set-main-photo"));
        if (shared) {
          flashButtonText(copyButton, "已发送");
        } else {
          showCopyFallback("系统不支持直接复制图片。请长按上方图片保存，或使用发送当前图片。", "复制当前图片");
          flashButtonText(copyButton, "系统拦截");
        }
      }
    });
  }
  if (shareButton) {
    shareButton.addEventListener("click", async () => {
      ...
      const shared = await sharePhoto(currentPhoto, container.querySelector(".set-main-photo"));
      if (!shared) {
        const text = toAbsoluteUrl(currentPhoto);
        const copied = await copyTextToClipboard(text);
        if (copied) {
          flashButtonText(shareButton, "已复制链接");
        } else {
          showCopyFallback(text, "发送当前图片");
          flashButtonText(shareButton, "手动复制");
        }
      }
    });
  }
