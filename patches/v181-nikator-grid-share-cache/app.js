...
const badNikatorQuickPhotoSeries = new Set([
  "LE8813SF",
  "LE8816SF",
  "LE8820SF",
  "LE8821SF",
...
function renderVersionBadge() {
  ...
  badge.textContent = `版本 ${versionCode} ${versionName} | ${patchText}`;
  anchor.after(badge);
}
...
function getQuickJumpPhoto(catalogKey, series, item) {
  ...
  if (catalogKey === "nikator") {
    ...
    if (badNikatorQuickPhotoSeries.has(key)) return placeholderImage();
    return overrides[series] || getNikatorGeneratedPhoto(series) || gallery.find(isWholeProductPhoto) || placeholderImage();
  }
...
function bindPhotoShare(container) {
  ...
  shareButton.addEventListener("click", async () => {
    const mainPhoto = container.querySelector(".set-main-photo");
    const src = getCurrentPhotoUrl(container);
    shareButton.disabled = true;
    shareButton.textContent = "分享中";
    const shared = await sharePhoto(src, mainPhoto);
    shareButton.disabled = false;
    if (shared) {
      flashButtonText(shareButton, "已打开分享");
      return;
    }
    flashButtonText(shareButton, "长按图片");
