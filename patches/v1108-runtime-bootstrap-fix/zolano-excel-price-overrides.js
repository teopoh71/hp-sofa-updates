...
(function forceZolano3792PhoneVisibility() {
  const series = "ZL 3792";
  const photo = "assets/generated/zolano/ZL3792.jpg";
  const materials = ["M/F", "F/SA", "NuBuck"];
  const rows = [
    { id: "ZL3792-1ER", configuration: "1ER", label: "1ER 1260mm", priceOptions: [9100, 14043, 15100] },
    { id: "ZL3792-1ERT", configuration: "1ER/T", label: "1ER/T 1260mm", priceOptions: [11300, 17391, 18700] },
    { id: "ZL3792-1NA", configuration: "1NA", label: "1NA 980mm", priceOptions: [7800, 11997, 12900] },
    { id: "ZL3792-1NAB", configuration: "1NAB", label: "1NAB 1450mm", priceOptions: [11400, 15624, 16800] },
    { id: "ZL3792-1NAT", configuration: "1NAT", label: "1NAT 1310mm", priceOptions: [8200, 13485, 14500] },
    { id: "ZL3792-TABLE", configuration: "TABLE", label: "TABLE 570mm", priceOptions: [2800, 4371, 4700] }
  ];
  ...
  function makeItem(row) {
    return {
      ...
      priceFactor: 1,
      priceIsFinal: true,
      source: "v1103-zolano3792-phone-visible",
      photo
    };
  }

  function upsertRows(target, front = false) {
    if (!Array.isArray(target)) return;
    rows.slice().reverse().forEach((row) => {
      const existing = target.find((item) =>
        String(item?.series || "") === series
        && String(item?.configuration || "").toUpperCase() === row.configuration.toUpperCase()
      );
      if (existing) {
        Object.assign(existing, makeItem(row), { id: existing.id || row.id });
        return;
      }
  ...
  function ensureRuntimeCatalog() {
    window.BAIDU_ZOLANO_SOFA_DATA ||= [];
    upsertRows(window.BAIDU_ZOLANO_SOFA_DATA, true);
    try {
      ...
      if (typeof catalogDefinitions !== "undefined" && catalogDefinitions.zolano?.catalog) {
        upsertRows(catalogDefinitions.zolano.catalog, true);
      }
      if (typeof catalogSofas !== "undefined" && Array.isArray(catalogSofas)) {
        upsertRows(catalogSofas, false);
      }
    } catch {
      // Older APKs may not expose all runtime variables; DOM repair below still keeps the button visible.
    }
  }
  ...
  function ensureButton() {
    ...
    if (!group) return;
    const wanted = 'zolano|ZL 3792';
    let button = group.querySelector(`[data-quick-jump="${wanted}"]`);
    if (!button) {
      button = document.createElement("button");
      button.type = "button";
      button.className = "quick-jump-button";
      button.dataset.quickJump = wanted;
      const image = document.createElement("img");
      image.src = photo;
      image.alt = "";
      image.loading = "eager";
      const text = document.createElement("span");
      text.textContent = "3792";
      button.append(image, text);
      const firstPage = group.querySelector(".quick-jump-page");
      if (firstPage) firstPage.prepend(button);
      else group.append(button);
    }
    const image = button.querySelector("img") || document.createElement("img");
    if (!image.parentElement) button.prepend(image);
    ...
    const page = button.closest(".quick-jump-pages");
    if (page) page.scrollLeft = 0;
    if (!button.dataset.zl3792ForceClick) {
      button.dataset.zl3792ForceClick = "1";
      button.addEventListener("click", () => {
        ensureRuntimeCatalog();
        try {
          if (typeof rebuildQuickJumpGroup === "function") rebuildQuickJumpGroup("zolano");
          if (typeof setActiveCatalog === "function" && typeof activeCatalogKey !== "undefined" && activeCatalogKey !== "zolano") {
            setActiveCatalog("zolano");
          }
        } catch {}
      });
    }
  ...
  function repair() {
    document.body?.setAttribute("data-zl3792-override", "v1105");
    ensureRuntimeCatalog();
    try {
      if (typeof rebuildQuickJumpGroup === "function") rebuildQuickJumpGroup("zolano");
      if (typeof syncQuickJumpImages === "function") {
        const group = document.querySelector('[data-quick-jump-group="zolano"]');
        if (group) syncQuickJumpImages(group);
      }
    } catch {}
    ensureButton();
  }
  ...
  const persistentRepair = setInterval(repair, 1000);
  setTimeout(() => clearInterval(persistentRepair), 30000);
}());
