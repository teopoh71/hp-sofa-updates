...
function ensureCatalogSwitchButtons() {
  ...
  if (backupButton) backupButton.textContent = "\u8336\u51e0";
  switchPanel?.querySelectorAll("button").forEach((button) => {
    if ((button.textContent || "").trim() === "\u5907\u7528") button.textContent = "\u8336\u51e0";
  });
...
async function maybeClearAppCache() {
  ...
  const clearKey = "hp-sofa-cache-cleared-v1021-coffee-table-fixed";
