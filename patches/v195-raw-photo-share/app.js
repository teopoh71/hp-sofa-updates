...
function renderVersionBadge() {
  ...
  quickSearch.addEventListener("input", () => {
    const target = document.querySelector("#searchInput") || document.querySelector("#builderSearchInput");
    if (!target) return;
...
function getCatalogQuickJumpItems(catalogKey, catalog) {
  ...
  if (catalogKey === "zolano") {
    ...
    return [...pinned, ...items.filter((item) => !pinnedSet.has(item.series)).slice(0, Math.max(0, limit - pinned.length))];
  }
...
async function maybeClearAppCache() {
  ...
  if (params.get("clearAppCache") !== "1" || sessionStorage.getItem("hp-sofa-cache-cleared") === "1") return;
  sessionStorage.setItem("hp-sofa-cache-cleared", "1");
  try {
    if ("serviceWorker" in navigator) {
...
function renderSetPreview() {
  ...
  setPhoto.innerHTML = `
    ...
    <div class="photo-actions">
      <button class="photo-action-button" type="button" data-share-current-photo>分享图片</button>
    </div>
...
function bindPhotoShare(container) {
  ...
  shareButton.addEventListener("click", () => {
    ...
    const src = selectedUrls[0] || getCurrentPhotoUrl(container);
    if (selectedUrls.length > 1) {
      openPhotoSelectionForLongPress(selectedUrls);
      return;
    }
...
async function sharePhotos(urls) {
  if (!navigator.share || !urls.length) return false;
  try {
    const files = [];
    for (let index = 0; index < Math.min(urls.length, 8); index += 1) {
      const blob = await getClipboardImageBlob(urls[index]);
      const extension = blob.type.includes("png") ? "png" : blob.type.includes("webp") ? "webp" : "jpg";
      files.push(new File([blob], `sofa-photo-${index + 1}.${extension}`, { type: blob.type || "image/jpeg" }));
    }
    if (navigator.canShare?.({ files })) {
      await navigator.share({ files, title: "\u6c99\u53d1\u56fe\u7247" });
      return true;
    }
  } catch {
    // Mobile browsers may block multi-file share from local app assets.
  }
  return false;
}
...
function openRawPhotoForLongPress(src) {
  window.location.href = toAbsoluteUrl(src);
}

function openPhotoSelectionForLongPress(urls) {
  const encoded = encodeURIComponent(JSON.stringify(urls.map(toAbsoluteUrl)));
  const url = `photo-viewer.html?srcs=${encoded}`;
