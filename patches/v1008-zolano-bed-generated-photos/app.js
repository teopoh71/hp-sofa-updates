...
async function maybeClearAppCache() {
  ...
  const clearKey = "hp-sofa-cache-cleared-v1008-zolano-bed-generated-photos";
...
function formatComboSummary(combo, selected, purchases = []) {
  if (purchases.length > 1) return `Ã¥Â·Â²Ã©â‚¬â€° ${purchases.length} Ã§Â»â€žÃ§Â»â€žÃ¥ÂË†`;
  if (purchases.length === 1 && purchases[0]?.combo) {
    const purchaseCombo = purchases[0].combo;
    return `${getComboPieceCount(purchaseCombo)}Ã¤Â»Â¶:${formatZolanoUnitConfig(purchaseCombo.configuration)}`;
  }
