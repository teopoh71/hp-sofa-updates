...
function isDiningTableItem(item) {
  ...
  return /Ã©Â¤ÂÃ¦Â¡Å’|Ã©Â¤ÂÃ¥ÂÂ°/.test(text) || /\bDT\b/i.test(`${item?.series || ""} ${item?.model || ""}`);
}

function isDiningChairItem(item) {
  const text = getCatalogText(item);
  return /Ã©Â¤ÂÃ¦Â¤â€¦/.test(text) || /\bCH\b/i.test(`${item?.series || ""} ${item?.model || ""}`);
}
...
function isDiningTurntableItem(item) {
  ...
  return /Ã¨Â½Â¬Ã§â€ºËœ/.test(text) || /^[Ãâ€ ÃŽÂ¦Ãâ€¢ÃÂ¤]\d{3,4}$/i.test(dimensions.trim());
}

function isBuyableDiningTableItem(item) {
  const text = getCatalogText(item);
  return /Ã©Â¤ÂÃ¦Â¡Å’|Ã©Â¤ÂÃ¥ÂÂ°/.test(text) && !isDiningTurntableItem(item);
}
...
function getDiningSizeValue(item) {
  const text = getCatalogText(item);
  const diameter = text.match(/[Ãâ€ ÃŽÂ¦Ãâ€¢ÃÂ¤](\d{3,5})/);
...
function getRelatedDiningTurntables(item) {
  ...
  const small = turntables.filter((candidate) => /Ã¥Â°ÂÃ¨Â½Â¬Ã§â€ºËœ/.test(getCatalogText(candidate)));
  const large = turntables.filter((candidate) => /Ã¥Â¤Â§Ã¨Â½Â¬Ã§â€ºËœ/.test(getCatalogText(candidate)));
...
function formatDiningTurntableNote(item) {
  ...
  if (!turntables.length) return "";
  return `Ã©â€¦ÂÃ¥Â¥â€”Ã¨Â½Â¬Ã§â€ºËœ:${turntables.map((turntable) => {
    ...
    .map((item) => {
      ...
      const correction = {
        ...
        "FULL SET": {
          ...
          description: "Ã¥â€ºÂ¾Ã§â€°â€¡Ã¦â€¢Â´Ã¥Â¥â€” MELFE",
        ...
        PILLOW: {
          description: "Pillow / Ã¦Å Â±Ã¦Å¾â€¢",
          dimensions: "Ã¦Å Â±Ã¦Å¾â€¢"
        }
      }[config];
...
const formatZolano3776LayoutDimension = zolano3776Layout.formatZolano3776LayoutDimension
  || ((layout) => `LÃ¥Â½Â¢Ã§Â»â€žÃ¥ÂË†:Ã¥Â·Â¦Ã¨Â¾Â¹ ${layout.left}mm / Ã¥ÂÂ³Ã¨Â¾Â¹ ${layout.right}mm | Ã¦Â·Â± ${layout.depth}mm | Ã©Â«Ëœ ${layout.height}mm`);
...
function ensureDynamicQuickJumpGroup(catalogKey) {
  ...
  group.setAttribute("aria-label", `${definition.label} Ã¥Å¾â€¹Ã¥ÂÂ·Ã¥â€ºÂ¾Ã§â€°â€¡`);
  group.hidden = true;
...
async function maybeClearAppCache() {
  ...
  const clearKey = "hp-sofa-cache-cleared-v263";
...
function showUpdateBanner(update) {
  ...
  title.textContent = update.mandatory ? "Ã¥Â¿â€¦Ã©Â¡Â»Ã¦â€ºÂ´Ã¦â€“Â°" : "Ã¦Å“â€°Ã¦â€“Â°Ã§â€°Ë†Ã¦Å“Â¬";

  const message = document.createElement("span");
  message.textContent = `${update.versionName || "Ã¦â€“Â°Ã§â€°Ë†Ã¦Å“Â¬"} Ã¥ÂÂ¯Ã¤Â»Â¥Ã¥Â®â€°Ã¨Â£â€¦`;
  ...
  updateButton.addEventListener("click", () => {
    ...
    alert("Ã¦â€ºÂ´Ã¦â€“Â°Ã©â€œÂ¾Ã¦Å½Â¥Ã¨Â¿ËœÃ¦Â²Â¡Ã¨Â®Â¾Ã§Â½Â®Ã£â‚¬â€šÃ¨Â¯Â·Ã¥â€¦Ë†Ã¥Å“Â¨ update.json Ã¥Â¡Â«Ã¥â€¦Â¥ apkUrlÃ£â‚¬â€š");
  });
...
function showPatchBanner(patch, manifestUrl) {
  ...
  updateButton.addEventListener("click", async () => {
    ...
    try {
      ...
      updateButton.textContent = "Ã¥Â®Å’Ã¦Ë†Â";
...
async function applyOnlinePatch(patch, manifestUrl, onProgress) {
  ...
  for (let index = 0; index < fetchedFiles.length; index += 1) {
    ...
    onProgress?.({ phase: "Ã¥Â®â€°Ã¨Â£â€¦", done: index + 1, total: fetchedFiles.length });
...
function render() {
  ...
  if (!filtered.length) {
    ...
    empty.textContent = allSofas.length ? "Ã¦Â²Â¡Ã¦Å“â€°Ã¦â€°Â¾Ã¥Ë†Â°Ã¥Å’Â¹Ã©â€¦ÂÃ§Å¡â€žÃ¦Â²â„¢Ã¥Ââ€˜Ã£â‚¬â€š" : "Ã¨Â¯Â·Ã¥â€¦Ë†Ã¦â€“Â°Ã¥Â¢Å¾Ã¦Â²â„¢Ã¥Ââ€˜Ã¥â€ºÂ¾Ã§â€°â€¡Ã¥â€™Å’Ã¤Â»Â·Ã¦Â Â¼Ã£â‚¬â€š";
  ...
  filtered.forEach((sofa) => {
    ...
    const titleText = sofa.name || sofa.model || "Ã¦Â²â„¢Ã¥Ââ€˜";
    ...
    [
      ...
      sofa.priceOptions?.length ? `${sofa.priceOptions.length} Ã¤Â¸ÂªÃ¦ÂÂÃ¨Â´Â¨Ã¤Â»Â·Ã¦Â Â¼` : ""
...
function initBuilder() {
  ...
  if (!series.length) {
    ...
    empty.value = "";
    empty.textContent = `${catalogDefinitions[activeCatalogKey].label} Ã§â€ºÂ®Ã¥Â½â€¢Ã¨Â¿ËœÃ¦Â²Â¡Ã¦Å“â€°Ã¨Âµâ€žÃ¦â€“â„¢`;
...
function populateModelJumpSelect(seriesList) {
  ...
  const empty = document.createElement("option");
  empty.value = "";
  empty.textContent = "Ã©â‚¬â€°Ã¦â€¹Â©Ã¥â€¦Â¨Ã©Æ’Â¨Ã¥Å¾â€¹Ã¥ÂÂ·";
...
function renderDiningTurntableSlot(seriesItems, retainedSelections = []) {
  ...
  label.textContent = "Ã¨Â½Â¬Ã§â€ºËœ";
...
function getComboElectricSeatText(combo) {
  ...
  return count ? `${count}Ã§â€ÂµÃ¥Å Â¨Ã¤Â½Â` : "";
}
...
function getComboElectricSeatCount(combo) {
  ...
  return String(combo?.description || "")
    .split(/[+Ã¯Â¼â€¹,Ã¯Â¼Å’Ã£â‚¬Â]/)
    .filter((part) => /Ã§â€ÂµÃ¥Å Â¨/.test(part))
    .length;
}

function isElectricSeatItem(item) {
  return /Ã§â€ÂµÃ¥Å Â¨|\bRL\b/i.test(`${item?.configuration || ""} ${item?.description || ""} ${item?.model || ""}`);
}
...
function getComboButtonDepthValue(combo) {
  ...
  const seatDepthMatch = text.match(/(?:seat|Ã¥ÂºÂ§Ã¦Â·Â±|Ã¥ÂÂÃ¦Â·Â±)\s*(?:D|depth)?\s*(\d{3,5})/i);
...
function matchesTypeFilters(combo) {
  ...
  const hasChaise = /(Ã¨Â´ÂµÃ¥Â¦Æ’|afl|afr|405|406|\ber\/t\b|\bel\/t\b|\bna\/t\b)/i.test(text);
...
function getComboPrimaryWidthValue(combo) {
  ...
  const closeArmMatch = text.match(/Ã¦â€Â¶Ã¦â€°Â¶Ã¦â€°â€¹\s*(\d{3,5})/i);
...
function getZolanoGeneratedRecommendations(items) {
  ...
  lefts.forEach((left) => {
    rights.forEach((right) => {
      generated.push(buildZolanoGeneratedCombo("Ã¥Â·Â¦Ã¥ÂÂ³Ã¦â€°Â¶Ã¦â€°â€¹Ã§Â»â€žÃ¥ÂË†", [left, right]));
      centers.forEach((center) => {
        generated.push(buildZolanoGeneratedCombo("Ã¥Â·Â¦Ã¥ÂÂ³Ã¦â€°Â¶Ã¦â€°â€¹Ã¥Å Â Ã¤Â¸Â­Ã¤Â½Â", [left, center, right]));
      });
    });
  });
...
function getZolano3776GeneratedRecommendations(items) {
  ...
  const presets = [
    ...
    {
      label: "ZL3776 Ã¥Â±â€¢Ã¥Å½â€¦Ã¦â€¢Â´Ã¥Â¥â€” + 1NA Ã¥Â·Â¦Ã¨Â¾Â¹Ã¥Å Â Ã©â€¢Â¿",
    ...
    {
      label: "ZL3776 Ã¥Â±â€¢Ã¥Å½â€¦Ã¦â€¢Â´Ã¥Â¥â€” + Ã¥Â·Â¦Ã¥ÂÂ³Ã¥Ââ€ž 1NA",
    ...
    { label: "ZL3776 Ã§ÂºÂ¦ 1m", parts: [oneSeat] },
    { label: "ZL3776 Ã§ÂºÂ¦ 1.5m", parts: [chaiseSeat] },
    { label: "ZL3776 Ã§ÂºÂ¦ 2.5m", parts: [chaiseSeat, oneSeat] },
    { label: "ZL3776 Ã§ÂºÂ¦ 2.6m", parts: [chaiseSeat, stool] },
    { label: "ZL3776 Ã§ÂºÂ¦ 2.9m", parts: [oneSeat, oneSeat, oneSeat] }
  ];
...
function buildZolanoGeneratedCombo(name, parts, options = {}) {
  ...
  combo.description = "Ã¦Å’â€°Ã¦â€°Â¶Ã¦â€°â€¹Ã§Â»â€žÃ¤Â»Â¶Ã§â€Å¸Ã¦Ë†ÂÃ§Â»â€žÃ¥ÂË†";
...
function getZolanoPartsDimensionText(parts) {
  ...
  if (hasAnyWidth && closeWidth && openWidth && closeWidth !== openWidth) {
    sizeParts.push(`Ã¦â€Â¶Ã¦â€°Â¶Ã¦â€°â€¹ ${closeWidth}mm / Ã¥Â¼â‚¬Ã¦â€°Â¶Ã¦â€°â€¹ ${openWidth}mm`);
  } else if (hasAnyWidth && closeWidth) {
    sizeParts.push(`Ã¥Â®Â½ ${closeWidth}mm`);
  }
  if (depth) sizeParts.push(`\u6df1 ${depth}mm`);
  if (height) sizeParts.push(`Ã©Â«Ëœ ${height}mm`);
  if (seatHeight) sizeParts.push(`Ã¥ÂÂÃ©Â«Ëœ ${seatHeight}mm`);
  return sizeParts.join(" | ");
}

function getZolanoItemWidthPair(item) {
  const text = String(item?.dimensions || "");
  const armMatch = text.match(/Ã¦â€Â¶Ã¦â€°Â¶Ã¦â€°â€¹\s*(\d{3,5})\s*\/\s*Ã¥Â¼â‚¬Ã¦â€°Â¶Ã¦â€°â€¹\s*(\d{3,5})/);
...
function getSeatHeightNumber(item) {
  const text = String(item?.dimensions || "");
  const match = text.match(/seat\s*H\s*(\d{3,5})|Ã¥ÂÂÃ©Â«Ëœ\s*(\d{3,5})/i);
...
function buildGeneratedCombo(name, parts) {
  ...
  return {
    ...
    description: "Ã¥Â¸Â¸Ã§â€Â¨Ã§â€ºÂ´Ã¦Å½â€™Ã§Â»â€žÃ¥ÂË†",
    partCodes,
...
function isZolanoAddOnItem(item) {
  ...
  return /\b(TABLE|PILLOW|HEADREST|USB|CUSHION)\b/.test(text) || /Ã¨Å’Â¶Ã¥â€¡Â |Ã¦Å Â±Ã¦Å¾â€¢|Ã¥Â¤Â´Ã¦Å¾â€¢|Ã©ÂÂ Ã¦Å¾â€¢|Ã©â€¦ÂÃ¤Â»Â¶|Ã¥Å Â Ã¨Â´Â­/.test(text);
}
...
function formatPieceTitle(item) {
  ...
  return `${isZolanoAddOnItem(item) ? "Ã©â€¦ÂÃ¤Â»Â¶/Ã¥Å Â Ã¨Â´Â­ - " : ""}${title || item?.model || item?.series || ""}`;
}
...
function formatPieceDimensionLine(item) {
  ...
  return dimensions && dimensions !== "Ã©â€¦ÂÃ¤Â»Â¶" ? dimensions : "";
}
...
function showCopyFallback(text, title = "Ã¥Â¤ÂÃ¥Ë†Â¶") {
  ...
  overlay.innerHTML = `
    <div class="copy-fallback-panel">
      <div class="copy-fallback-heading">
        <strong>${title}</strong>
        <button type="button" data-close-copy-fallback>Ã¥â€¦Â³Ã©â€”Â­</button>
      </div>
      <p>Ã¨â€¡ÂªÃ¥Å Â¨Ã¥Â¤ÂÃ¥Ë†Â¶Ã¨Â¢Â«Ã§Â³Â»Ã§Â»Å¸Ã¦â€¹Â¦Ã¦Ë†ÂªÃ¯Â¼Å’Ã¨Â¯Â·Ã©â€¢Â¿Ã¦Å’â€°Ã¤Â¸â€¹Ã©ÂÂ¢Ã¥â€ â€¦Ã¥Â®Â¹Ã¥Â¤ÂÃ¥Ë†Â¶Ã£â‚¬â€š</p>
...
function openFullscreenPhoto(src) {
  ...
  overlay.innerHTML = `
    <button class="photo-fullscreen-close" type="button" aria-label="\u653e\u5927\u56fe\u7247">Ãƒâ€”</button>
    <button class="photo-fullscreen-raw" type="button" data-open-raw-photo>Ã¦â€°â€œÃ¥Â¼â‚¬Ã¥â€ºÂ¾Ã§â€°â€¡</button>
    <img src="${src}" alt="Ã¦Â²â„¢Ã¥Ââ€˜Ã¥â€ºÂ¾Ã§â€°â€¡" data-open-raw-photo>
  `;
  overlay.addEventListener("click", (event) => {
...
function formatComboOption(combo) {
  ...
  const isSlant = /-B\b|Ã¦â€“Å“Ã¤Â½Â|Ã¦â€“Å“Ã¥Â½Â¢/i.test(combo.configuration || "");
  const nameLabel = isSlant ? `${combo.name} (Ã¦â€“Å“Ã¥Â½Â¢)` : combo.name;
...
function getZolano2897LShapeDimension(selected) {
  ...
  const cornerIndex = layoutItems.findIndex((item) =>
    /CNR|CORNER|Ã¨Â½Â¬Ã¨Â§â€™/i.test(`${item.configuration || ""} ${item.description || ""}`)
  );
...
function getDiningMaterialGradeKey(item) {
  const text = typeof item === "string" ? item : getCatalogText(item);
  if (/Ã¤Â¸â‚¬Ã§Â­â€°Ã§ÂºÂ§/.test(text)) return "Ã¤Â¸â‚¬Ã§Â­â€°Ã§ÂºÂ§";
  if (/Ã¤ÂºÅ’Ã§Â­â€°Ã§ÂºÂ§/.test(text)) return "Ã¤ÂºÅ’Ã§Â­â€°Ã§ÂºÂ§";
  if (/Ã¤Â¸â€°Ã§Â­â€°Ã§ÂºÂ§/.test(text)) return "Ã¤Â¸â€°Ã§Â­â€°Ã§ÂºÂ§";
  return "";
}
...
function extractDiningStoneNames(text) {
  ...
  const match = normalized.match(/[Ã¤Â¸â‚¬Ã¤ÂºÅ’Ã¤Â¸â€°]Ã§Â­â€°Ã§ÂºÂ§[::]\s*([^Ã¦Â¡â€ Ã¥Âºâ€¢Ã¨â€žÅ¡]+)/);
  if (!match) return "";
  return match[1].replace(/[Ã£â‚¬â€š;;]+$/g, "").trim();
}
...
function isCenterPiece(item) {
  const text = `${item.configuration} ${item.description}`.toUpperCase();
  return text.includes("CB") || text.includes("Ã¦â€”Â Ã¦â€°Â¶Ã¦â€°â€¹") || text.includes("CENTER");
}

function isRightPiece(item) {
  const text = `${item.configuration} ${item.description}`.toUpperCase();
  return text.includes("PR") || text.includes("AFR") || text.includes("CSAR") || text.includes("Ã¥ÂÂ³");
}
...
function deleteSofa(id) {
  if (!confirm("Ã§Â¡Â®Ã¥Â®Å¡Ã¨Â¦ÂÃ¥Ë†Â Ã©â„¢Â¤Ã¨Â¿â„¢Ã¤Â¸ÂªÃ¦Â²â„¢Ã¥Ââ€˜Ã¥Ââ€”?")) return;
...
function showPhotoPreview(src) {
  ...
  const image = document.createElement("img");
  image.src = src;
  image.alt = "Ã¥Â·Â²Ã©â‚¬â€°Ã¦Â²â„¢Ã¥Ââ€˜Ã¥â€ºÂ¾Ã§â€°â€¡";
