...
const zolanoQuickPhotoOverrides = new Map([
  ...
  ["ZL-523", "assets/generated/zolano-3digit-line/ZL523.svg"],
  ["ZL-556", "assets/generated/zolano-3digit-line/ZL556.svg"],
  ["ZL-571", "assets/generated/zolano-3digit-line/ZL571.svg"]
]);
...
const zolanoThreeDigitCatalog = [
  ...
  },
  {
    ...
    priceOptions: [0],
    materials: ["Zolano"],
    photo: "assets/generated/zolano-3digit-line/ZL523.svg",
  ...
  },
  {
    ...
    priceOptions: [0],
    materials: ["Zolano"],
    photo: "assets/generated/zolano-3digit-line/ZL556.svg",
    source: "manual-armchair-list"
  },
  {
  ...
  },
  {
    ...
    source: "https://www.zolano.com/product/luxon/"
  },
  {
    ...
    priceOptions: [8390],
    materials: ["Zolano"],
    photo: "assets/generated/zolano-3digit-line/ZL571.svg",
...
const zolano2628Modules = [
  ...
  },
  {
    ...
    label: "1ER/T",
    meta: "910mm",
    photo: "assets/generated/zolano/parts/MONTIERIZL2628-2ER-top.png"
  },
  {
...
const zolanoModulePhotoMap = Object.fromEntries([...zolano2897Modules, ...zolano3776Modules, ...zolano2628Modules].map((module) => [module.id, module.photo]));
const nikatorModuleSeriesSet = new Set(["LE8801SF", "LE8810SF", "LE8806SF", "NK0051SF", "NK0054SF"]);
...
function isDiningTableItem(item) {
  ...
  return /é¤æ¡Œ|é¤å°/.test(text) || /\bDT\b/i.test(`${item?.series || ""} ${item?.model || ""}`);
}

function isDiningChairItem(item) {
  const text = getCatalogText(item);
  return /é¤æ¤…/.test(text) || /\bCH\b/i.test(`${item?.series || ""} ${item?.model || ""}`);
}
...
function isDiningTurntableItem(item) {
  ...
  return /è½¬ç›˜/.test(text) || /^[Ï†Î¦Ï•Ð¤]\d{3,4}$/i.test(dimensions.trim());
}

function isBuyableDiningTableItem(item) {
  const text = getCatalogText(item);
  return /é¤æ¡Œ|é¤å°/.test(text) && !isDiningTurntableItem(item);
}
...
function getDiningSizeValue(item) {
  const text = getCatalogText(item);
  const diameter = text.match(/[Ï†Î¦Ï•Ð¤](\d{3,5})/);
...
function getRelatedDiningTurntables(item) {
  ...
  const small = turntables.filter((candidate) => /å°è½¬ç›˜/.test(getCatalogText(candidate)));
  const large = turntables.filter((candidate) => /å¤§è½¬ç›˜/.test(getCatalogText(candidate)));
...
function formatDiningTurntableNote(item) {
  ...
  if (!turntables.length) return "";
  return `é…å¥—è½¬ç›˜:${turntables.map((turntable) => {
...
const formatZolano3776LayoutDimension = zolano3776Layout.formatZolano3776LayoutDimension
  || ((layout) => `Lå½¢ç»„åˆ:å·¦è¾¹ ${layout.left}mm / å³è¾¹ ${layout.right}mm | æ·± ${layout.depth}mm | é«˜ ${layout.height}mm`);
...
function syncQuickJumpVisibility() {
  ...
  if (activeCatalogKey === "nikator") rebuildQuickJumpGroup("nikator");
...
function ensureDynamicQuickJumpGroup(catalogKey) {
  ...
  if (existingGroup) {
    if ((catalogKey === "nikator" || catalogKey === "zolano" || catalogKey === "bed") && !existingGroup.querySelector(".quick-jump-pages")) {
  ...
  group.setAttribute("aria-label", `${definition.label} åž‹å·å›¾ç‰‡`);
  group.hidden = true;
...
async function maybeClearAppCache() {
  ...
  const clearKey = "hp-sofa-cache-cleared-v261";
...
function showUpdateBanner(update) {
  ...
  title.textContent = update.mandatory ? "å¿…é¡»æ›´æ–°" : "æœ‰æ–°ç‰ˆæœ¬";

  const message = document.createElement("span");
  message.textContent = `${update.versionName || "æ–°ç‰ˆæœ¬"} å¯ä»¥å®‰è£…`;
  ...
  updateButton.addEventListener("click", () => {
    ...
    alert("æ›´æ–°é“¾æŽ¥è¿˜æ²¡è®¾ç½®ã€‚è¯·å…ˆåœ¨ update.json å¡«å…¥ apkUrlã€‚");
  });
...
async function checkForOnlinePatch() {
  ...
  try {
    ...
    const installedPatchCode = Math.max(
      ...
      Number(currentAppVersion.patchVersionCode || 0)
    );
...
function showPatchBanner(patch, manifestUrl) {
  ...
  updateButton.addEventListener("click", async () => {
    ...
    try {
      ...
      updateButton.textContent = "å®Œæˆ";
...
async function applyOnlinePatch(patch, manifestUrl, onProgress) {
  ...
  for (let index = 0; index < fetchedFiles.length; index += 1) {
    ...
    onProgress?.({ phase: "å®‰è£…", done: index + 1, total: fetchedFiles.length });
...
function render() {
  ...
  if (!filtered.length) {
    ...
    empty.textContent = allSofas.length ? "æ²¡æœ‰æ‰¾åˆ°åŒ¹é…çš„æ²™å‘ã€‚" : "è¯·å…ˆæ–°å¢žæ²™å‘å›¾ç‰‡å’Œä»·æ ¼ã€‚";
  ...
  filtered.forEach((sofa) => {
    ...
    const titleText = sofa.name || sofa.model || "æ²™å‘";
    ...
    [
      ...
      sofa.priceOptions?.length ? `${sofa.priceOptions.length} ä¸ªæè´¨ä»·æ ¼` : ""
...
function initBuilder() {
  ...
  if (!series.length) {
    ...
    empty.value = "";
    empty.textContent = `${catalogDefinitions[activeCatalogKey].label} ç›®å½•è¿˜æ²¡æœ‰èµ„æ–™`;
...
function populateModelJumpSelect(seriesList) {
  ...
  const empty = document.createElement("option");
  empty.value = "";
  empty.textContent = "é€‰æ‹©å…¨éƒ¨åž‹å·";
...
function getComboElectricSeatCount(combo) {
  ...
  return String(combo?.description || "")
    .split(/[+ï¼‹,ï¼Œã€]/)
    .filter((part) => /ç”µåŠ¨/.test(part))
    .length;
}

function isElectricSeatItem(item) {
  return /ç”µåŠ¨|\bRL\b/i.test(`${item?.configuration || ""} ${item?.description || ""} ${item?.model || ""}`);
}
...
function getComboButtonDepthValue(combo) {
  ...
  const seatDepthMatch = text.match(/(?:seat|åº§æ·±|åæ·±)\s*(?:D|depth)?\s*(\d{3,5})/i);
...
function isZolanoModuleSeriesName(seriesValue) {
  ...
  return /(?:^|[^0-9])(?:2628|2897|3776)(?:[^0-9]|$)/.test(text);
}
...
function syncZolanoModulePicker() {
  ...
  zolanoModulePicker.innerHTML = `
    ...
    <div class="module-picker-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(72px,1fr));gap:5px;">
      ${modules.map((module) => `
        <button class="module-picker-button" type="button" data-zolano-module-id="${module.id}" style="display:grid;grid-template-rows:36px auto auto;gap:1px;min-height:72px;padding:4px 3px;border-radius:6px;">
          <img src="${module.photo}" alt="${module.label}" style="width:100%;height:36px;object-fit:contain;">
...
function matchesTypeFilters(combo) {
  if (selectedTypeFilters.has("showroom")) return isShowroomCombination(combo);
  const text = [
    combo.name,
    combo.configuration,
    combo.description
  ].join(" ").toLowerCase();
  const hasChaise = /(è´µå¦ƒ|afl|afr|405|406|\ber\/t\b|\bel\/t\b|\bna\/t\b)/i.test(text);
...
function getComboPrimaryWidthValue(combo) {
  ...
  const closeArmMatch = text.match(/æ”¶æ‰¶æ‰‹\s*(\d{3,5})/i);
...
function getZolanoGeneratedRecommendations(items) {
  ...
  lefts.forEach((left) => {
    rights.forEach((right) => {
      generated.push(buildZolanoGeneratedCombo("å·¦å³æ‰¶æ‰‹ç»„åˆ", [left, right]));
      centers.forEach((center) => {
        generated.push(buildZolanoGeneratedCombo("å·¦å³æ‰¶æ‰‹åŠ ä¸­ä½", [left, center, right]));
      });
    });
  });
...
function getZolano3776GeneratedRecommendations(items) {
  ...
  const presets = [
    ...
    {
      label: "ZL3776 å±•åŽ…æ•´å¥— + 1NA å·¦è¾¹åŠ é•¿",
    ...
    {
      label: "ZL3776 å±•åŽ…æ•´å¥— + å·¦å³å„ 1NA",
    ...
    { label: "ZL3776 çº¦ 1m", parts: [oneSeat] },
    { label: "ZL3776 çº¦ 1.5m", parts: [chaiseSeat] },
    { label: "ZL3776 çº¦ 2.5m", parts: [chaiseSeat, oneSeat] },
    { label: "ZL3776 çº¦ 2.6m", parts: [chaiseSeat, stool] },
    { label: "ZL3776 çº¦ 2.9m", parts: [oneSeat, oneSeat, oneSeat] }
  ];
...
function buildZolanoGeneratedCombo(name, parts, options = {}) {
  ...
  combo.description = "æŒ‰æ‰¶æ‰‹ç»„ä»¶ç”Ÿæˆç»„åˆ";
...
function getZolanoPartsDimensionText(parts) {
  ...
  if (hasAnyWidth && closeWidth && openWidth && closeWidth !== openWidth) {
    sizeParts.push(`æ”¶æ‰¶æ‰‹ ${closeWidth}mm / å¼€æ‰¶æ‰‹ ${openWidth}mm`);
  } else if (hasAnyWidth && closeWidth) {
    sizeParts.push(`å®½ ${closeWidth}mm`);
  }
  if (depth) sizeParts.push(`\u6df1 ${depth}mm`);
  if (height) sizeParts.push(`é«˜ ${height}mm`);
  if (seatHeight) sizeParts.push(`åé«˜ ${seatHeight}mm`);
  return sizeParts.join(" | ");
}

function getZolanoItemWidthPair(item) {
  const text = String(item?.dimensions || "");
  const armMatch = text.match(/æ”¶æ‰¶æ‰‹\s*(\d{3,5})\s*\/\s*å¼€æ‰¶æ‰‹\s*(\d{3,5})/);
...
function getSeatHeightNumber(item) {
  const text = String(item?.dimensions || "");
  const match = text.match(/seat\s*H\s*(\d{3,5})|åé«˜\s*(\d{3,5})/i);
...
function isZolanoAddOnItem(item) {
  ...
  return /\b(TABLE|PILLOW|HEADREST|USB|CUSHION)\b/.test(text) || /èŒ¶å‡ |æŠ±æž•|å¤´æž•|é æž•|é…ä»¶|åŠ è´­/.test(text);
}
...
function formatPieceTitle(item) {
  ...
  return `${isZolanoAddOnItem(item) ? "é…ä»¶/åŠ è´­ - " : ""}${title || item?.model || item?.series || ""}`;
}
...
function formatPieceDimensionLine(item) {
  ...
  return dimensions && dimensions !== "é…ä»¶" ? dimensions : "";
}
...
function showCopyFallback(text, title = "å¤åˆ¶") {
  ...
  overlay.innerHTML = `
    <div class="copy-fallback-panel">
      <div class="copy-fallback-heading">
        <strong>${title}</strong>
        <button type="button" data-close-copy-fallback>å…³é—­</button>
      </div>
      <p>è‡ªåŠ¨å¤åˆ¶è¢«ç³»ç»Ÿæ‹¦æˆªï¼Œè¯·é•¿æŒ‰ä¸‹é¢å†…å®¹å¤åˆ¶ã€‚</p>
...
function openFullscreenPhoto(src) {
  ...
  overlay.innerHTML = `
    <button class="photo-fullscreen-close" type="button" aria-label="\u653e\u5927\u56fe\u7247">Ã—</button>
    <button class="photo-fullscreen-raw" type="button" data-open-raw-photo>æ‰“å¼€å›¾ç‰‡</button>
    <img src="${src}" alt="æ²™å‘å›¾ç‰‡" data-open-raw-photo>
  `;
  overlay.addEventListener("click", (event) => {
...
function formatComboOption(combo) {
  ...
  const isSlant = /-B\b|æ–œä½|æ–œå½¢/i.test(combo.configuration || "");
  const nameLabel = isSlant ? `${combo.name} (æ–œå½¢)` : combo.name;
...
function getZolano2897LShapeDimension(selected) {
  ...
  const cornerIndex = layoutItems.findIndex((item) =>
    /CNR|CORNER|è½¬è§’/i.test(`${item.configuration || ""} ${item.description || ""}`)
  );
...
function getZolano2628LShapeDimension(selected) {
  ...
  const suffix = depth ? ` | æ·± ${depth}mm` : "";
  return sideA && sideB ? `Lå½¢ç»„åˆï¼šå·¦è¾¹ ${sideA}mm / å³è¾¹ ${sideB}mm${suffix}` : "";
}
...
function getZolano3776LShapeDimension(selected) {
  ...
  const cornerIndex = layoutItems.findIndex((item) =>
    /CORNER|è½¬è§’/i.test(`${item.configuration || ""} ${item.description || ""}`)
  );
  if (cornerIndex < 0) return "";
  ...
  const suffix = depth ? ` | æ·± ${depth}mm` : "";
  return sideA && sideB && height ? `Lå½¢ç»„åˆï¼šå·¦è¾¹ ${sideA}mm / å³è¾¹ ${sideB}mm${suffix} | é«˜ ${height}mm` : "";
}
...
function getMaterialPrice(item, index) {
  ...
  if (activeCatalogKey === "zolano" && item?.priceIsFinal) {
...
function getDiningMaterialGradeKey(item) {
  const text = typeof item === "string" ? item : getCatalogText(item);
  if (/ä¸€ç­‰çº§/.test(text)) return "ä¸€ç­‰çº§";
  if (/äºŒç­‰çº§/.test(text)) return "äºŒç­‰çº§";
  if (/ä¸‰ç­‰çº§/.test(text)) return "ä¸‰ç­‰çº§";
  return "";
}
...
function extractDiningStoneNames(text) {
  ...
  const match = normalized.match(/[ä¸€äºŒä¸‰]ç­‰çº§[::]\s*([^æ¡†åº•è„š]+)/);
  if (!match) return "";
  return match[1].replace(/[ã€‚;;]+$/g, "").trim();
}
...
function isCenterPiece(item) {
  const text = `${item.configuration} ${item.description}`.toUpperCase();
  return text.includes("CB") || text.includes("æ— æ‰¶æ‰‹") || text.includes("CENTER");
}

function isRightPiece(item) {
  const text = `${item.configuration} ${item.description}`.toUpperCase();
  return text.includes("PR") || text.includes("AFR") || text.includes("CSAR") || text.includes("å³");
}
...
function deleteSofa(id) {
  if (!confirm("ç¡®å®šè¦åˆ é™¤è¿™ä¸ªæ²™å‘å—?")) return;
